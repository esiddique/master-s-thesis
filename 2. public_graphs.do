...
gen public=(NBP_per>0)

collapse (mean) gross_sales cogs profit market_share profit_margin, by(public year)
...
export excel "$output/graphs/public_fertilizer.xlsx", sheet("raw") sheetreplace first(var)
export delimited "$output/public_fertilizer.csv", replace 

*** FERTILIZER OUTPUT ***
use "$output/fertilizer_2", clear
gen public=(NBP_per>0)

collapse (mean) output, by(public year)
...
export delimited "$output/public_fertilizer_output.csv", replace 
...
export excel "$output/graphs/public_cement.xlsx", sheet("raw") sheetreplace first(var)
export delimited "$output/public_cement.csv", replace 
